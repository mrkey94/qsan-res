...
sgs.ai_skill_use["@@jieyue"] = function(self, prompt, method)
  if self.player:isKongcheng() or (self.player:containsTrick("SupplyShortage") and self:getCardsNum("Nullification") == 0) then
    return "."
  end
...
local function shouldUseXuanhuo(self)
  ...
  if table.contains(xuanhuosklii,"kuanggu") and self:getCardsNum("Slash") < 2 and
  ...
  if not target then--无杀目标或无杀
    --assert(target)
    self.room:writeToConsole("眩惑无杀目标或无杀返回:"..self.player:objectName())
    return false
  end
  assert(target)
...
sgs.ai_skill_use_func.XuanhuoAttachCard= function(card, use, self)
  ...
  if self.player:hasSkill("jizhi") then--使用锦囊后
    ...
    if self.need_kaunggu_AOE then--使用AOE前
...
sgs.ai_skill_choice.xuanhuo = function(self, choices)
  ...
  sgs.debugFunc(self.player, 2)
  local target = getSlashtarget(self)--中间给牌弃牌，可能失去武器或杀导致无返回目标

  if not target then
    self.room:writeToConsole("眩惑选择无杀目标或无杀")
    --assert(target)
  ...
  global_room:writeToConsole("选取技能:" .. self.player:objectName() )
...
sgs.ai_skill_choice["startcommand_buyi"] = function(self, choices)
  self.player:speak(choices)
  choices = choices:split("+")
  local commands = {"command1", "command2", "command4", "command3", "command6", "command5"}--索引大小代表优先级，注意不是原顺序
  local command_value1 = table.indexOf(commands,choices[1])
  local command_value2 = table.indexOf(commands,choices[2])
  self.buyi_index = math.max(command_value1,command_value2)--需要一些额外的标记？
  --global_room:writeToConsole("choice:".. choices[self.buyi_index])
  return commands[self.buyi_index]
end

sgs.ai_skill_choice["docommand_buyi"] = function(self, choices, data)
  --[[if self.buyi_index <= 4 then
    return "yes"
  end
  if self.buyi_index == 5 then
    if self.player:getCards("he"):length() < 4 then
      return "yes"
    elseif self.player:getHp() < 3 and self:getCardsNum("Peach") == 0 and self.player:getCards("he"):length() < 6 then
      return "yes"
