	...
	for _, c in sgs.qlist(self.player:getCards("he")) do
		if c:isTransferable() and (not isCard("Peach", c, self.player) or #friends_shown > 0) then
			if not oneJink and isCard("Jink", c, self.player) then
				oneJink = true
				continue
			elseif self.player:getHp() == 1 and isCard("Analeptic", c, self.player) then
				continue
			...
			elseif string.find(gameProcess, self.player:getKingdom() .. ">>")  then--均势？
				...
				for _, card in ipairs(cards) do	
					if card:isKindOf("BurningCamps") then--大国？火烧联营不能给
						table.removeOne(cards, card)
						global_room:writeToConsole(">>> 大国？火烧联营不能给")
					end
				end
			end
		end
...
function SmartAI:useCardDrowning(card, use)
	...
	for _, friend in ipairs(self.friends_noself) do
		if card:targetFilter(players, friend, self.player) and not players:contains(friend) and (self:needToThrowArmor(friend) or friend:hasShownSkills(sgs.lose_equip_skill)) and friend:getEquips():length() == 1 then
