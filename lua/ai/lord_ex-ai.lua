...
sgs.ai_skill_invoke.qiuan = function(self, data)
  ...
  if self.player:hasSkills(sgs.masochism_skill) and self.player:getHp() > 1 then
    return false
  end
	return true
end
...
sgs.ai_skill_invoke.pozhen = function(self, data)
  ...
  if not self:isFriend(target) then
    if target:getOverflow() > 2 or (self.player:getHp() == 1 and self:getAllPeachNum() < 1) then
...
--[[
  ...
  self:useCardLureTiger(sgs.cloneCard("LureTiger"), dummyuse)
...
sgs.ai_skill_invoke.jilix = function(self, data)
  local prompt = data:toStringList()
...
sgs.ai_skill_playerchosen.huaiyi = function(self, targets, max_num, min_num)
  ...
  global_room:writeToConsole("预定目标数:"..#result)
  return result
end
